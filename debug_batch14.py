#!/usr/bin/env python3
"""
Debug - call model directly to see if it's deterministic.
"""
...
atomic_numbers_ch4 = torch.tensor(mol_ch4.get_atomic_numbers(), dtype=torch.long, device='cuda')
positions_ch4 = torch.tensor(mol_ch4.get_positions(), dtype=torch.float32, device='cuda')
...
atomic_numbers_h2o = torch.tensor(mol_h2o.get_atomic_numbers(), dtype=torch.long, device='cuda')
positions_h2o = torch.tensor(mol_h2o.get_positions(), dtype=torch.float32, device='cuda')
...
atomic_numbers_nh3 = torch.tensor(mol_nh3.get_atomic_numbers(), dtype=torch.long, device='cuda')
positions_nh3 = torch.tensor(mol_nh3.get_positions(), dtype=torch.float32, device='cuda')
...
# Compare
print("\nComparison:")
print(f"  Test 1 vs Test 2: energy_diff={abs(energy1.item() - energy2.item()):.2e}, forces_diff={torch.max(torch.abs(forces1 - forces2)).item():.2e}")
print(f"  Test 1 vs Test 3: energy_diff={abs(energy1.item() - energy3.item()):.2e}, forces_diff={torch.max(torch.abs(forces1 - forces3)).item():.2e}")
