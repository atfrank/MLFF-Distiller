#!/usr/bin/env python3
"""
FINAL CORRECT Training Script for Tiny and Ultra-tiny Models

Uses DistillationWrapper approach - exactly like the original student training.
...
def train_epoch(model, dataloader, optimizer, device, force_weight=15.0):
    ...
    for batch in pbar:
        ...
        # Student predictions via wrapper (handles force computation)
        predictions = model(batch)
        student_energy = predictions['energy']
        student_forces = predictions['forces']
...
def train_model(
    ...
    # Wrap with DistillationWrapper (like original training)
    model = DistillationWrapper(student).to(device)
    ...
    for epoch in range(epochs):
        ...
        if val_losses['total'] < best_val_loss:
            ...
            checkpoint = {
                'epoch': epoch,
                'model_state_dict': student.state_dict(),  # Save unwrapped student
                'config': config,
...
def main():
    ...
    print(f"\n{'='*70}")
    print("COMPACT MODEL TRAINING - DISTILLATION WRAPPER APPROACH")
    print(f"{'='*70}")
